...
const BlogClientComponent = () => {
  ...
  const fetchPopularBlogList = async () => {
    try {
      const response = await requestHandler(
        `/getMostpopular/?pageIndex=${page}&pageSize=10&search=${search}`,
        {},
        "get"
      );
  ...
  const fetchRecentBlogList = async () => {
    try {
      const response = await requestHandler(
        `/getRecentBlog/?pageIndex=${page}&pageSize=10&search=${search}`,
        {},
        "get"
      );
      console.log(response.data); // Log for debugging
          ...
          <main className="blog-header px-0">
            ...
            <div className="btn-groups">
              <button
                ...
                onClick={fetchRecentBlogList}
              >
                Most Recent
              </button>
              <button
                className="most-pplr-btn button"
                onClick={fetchPopularBlogList}
              >
                Most Popular
              </button>
            </div>
                      ...
                      <div key={blog.id} className="image-placeholder-parent">
                        <div className="img-wrapper">
                          <Image
                            ...
                            loading="lazy"
                            alt={blog.title}
                            src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${blog.bannerImage}`} // Fallback image
                          />
                        </div>
                        <div className="frame-group">
                          <div className="heading-parent">
                            <Link href={`/${blog.slug}`}>
                              <h2 className="heading">{blog.bannerTitle}</h2>
                            </Link>
                            {!blog.wordpress ? <p
                              className="dec mb-2"
...
function FilteredContent({data}:{data:any}) {
  const des = data?.content[0].split("\u003Ch2\u003E");
  const description = des[0].replace( /Introduction/i, "")
  return(
    <p
    className="dec mb-2"
    dangerouslySetInnerHTML={{
