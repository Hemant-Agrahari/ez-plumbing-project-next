...
const Page = () => {
  ...
  const handleEdit = (row: any) => {321
    Router.push(`/tag-management/view/view?tagId=${row.original.id}`);
  };
  ...
  const fetchData = async () => {
    const response = await requestHandler(
      `/tags/list?pageIndex=${pagination.pageIndex + 1}&pageSize=10000&search=${inputQuery}`,
      {},
      "get"
    );
  ...
  return (
    <>
      <Navigation profile={`Tag Management`}
